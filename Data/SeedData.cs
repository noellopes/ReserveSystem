...
namespace ReserveSystem.Data
{
    public static class SeedData
    {
        ...
        private const string ReservationistRole = "Reservationist";
            ...
            UserManager<IdentityUser> userManager, ILogger logger)
        {
            ...
            await SeedRolesAndUsersAsync(roleManager, userManager, logger).ConfigureAwait(false);
            await SeedTipoEquipamentoAsync(db, logger).ConfigureAwait(false);
            await SeedEquipamentoAsync(db, logger).ConfigureAwait(false);
            await SeedTipoReservaAsync(db, logger).ConfigureAwait(false);
            await SeedClientModelAsync(db, logger).ConfigureAwait(false);
            await SeedTipoSalaAsync(db, logger).ConfigureAwait(false);
            await SeedSalaAsync(db, logger).ConfigureAwait(false);
            await SeedReservaAsync(db, logger).ConfigureAwait(false);
        }

        private static async Task SeedRolesAndUsersAsync(RoleManager<IdentityRole> roleManager,
            UserManager<IdentityUser> userManager, ILogger logger)
        {
            var roles = new[] { ReservationistRole, ManagerRole, ClientRole };
            ...
            await EnsureUserWithRoleAsync(userManager, "manager@example.com", DefaultPassword, ManagerRole, logger).ConfigureAwait(false);
            await EnsureUserWithRoleAsync(userManager, "reservationist@example.com", DefaultPassword, ReservationistRole, logger).ConfigureAwait(false);
            await EnsureUserWithRoleAsync(userManager, "client@example.com", DefaultPassword, ClientRole, logger).ConfigureAwait(false);
        }

        private static async Task EnsureUserWithRoleAsync(UserManager<IdentityUser> userManager, string email,
            string password, string role, ILogger logger)
        {
            ...
            if (!await userManager.IsInRoleAsync(user, role).ConfigureAwait(false))
            {
                ...
                logger.LogInformation($"User '{email}' added to role '{role}'.");
            }
        }

        private static async Task SeedTipoEquipamentoAsync(ReserveSystemContext context, ILogger logger)
        {
            if (await context.TipoEquipamento.AnyAsync().ConfigureAwait(false)) return;
            ...
            await context.TipoEquipamento.AddRangeAsync(equipmentTypes).ConfigureAwait(false);
            await context.SaveChangesAsync().ConfigureAwait(false);
            logger.LogInformation("TipoEquipamento seeded.");
        }

        private static async Task SeedEquipamentoAsync(ReserveSystemContext context, ILogger logger)
        {
            if (await context.Equipamento.AnyAsync().ConfigureAwait(false)) return;
            ...
            await context.Equipamento.AddRangeAsync(equipment).ConfigureAwait(false);
            await context.SaveChangesAsync().ConfigureAwait(false);
            logger.LogInformation("Equipamento seeded.");
        }

        private static async Task SeedTipoReservaAsync(ReserveSystemContext context, ILogger logger)
        {
            if (await context.TipoReserva.AnyAsync().ConfigureAwait(false)) return;
            ...
            await context.TipoReserva.AddRangeAsync(tipoReservas).ConfigureAwait(false);
            await context.SaveChangesAsync().ConfigureAwait(false);
            logger.LogInformation("TipoReserva seeded.");
        }

        private static async Task SeedClientModelAsync(ReserveSystemContext context, ILogger logger)
        {
            if (await context.ClientModel.AnyAsync().ConfigureAwait(false)) return;
            ...
            await context.ClientModel.AddRangeAsync(clients).ConfigureAwait(false);
            await context.SaveChangesAsync().ConfigureAwait(false);
            logger.LogInformation("ClientModel seeded.");
        }

        private static async Task SeedTipoSalaAsync(ReserveSystemContext context, ILogger logger)
        {
            if (await context.TipoSala.AnyAsync().ConfigureAwait(false)) return;
            ...
            await context.TipoSala.AddRangeAsync(tipoSalas).ConfigureAwait(false);
            await context.SaveChangesAsync().ConfigureAwait(false);
            logger.LogInformation("TipoSala seeded.");
        }

        private static async Task SeedSalaAsync(ReserveSystemContext context, ILogger logger)
        {
            if (await context.Sala.AnyAsync().ConfigureAwait(false)) return;
            ...
            await context.Sala.AddRangeAsync(salas).ConfigureAwait(false);
            await context.SaveChangesAsync().ConfigureAwait(false);
            logger.LogInformation("Sala seeded.");
        }

        private static async Task SeedReservaAsync(ReserveSystemContext context, ILogger logger)
        {
            if (await context.Reserva.AnyAsync().ConfigureAwait(false)) return;
            ...
            await context.Reserva.AddRangeAsync(reservas).ConfigureAwait(false);
            await context.SaveChangesAsync().ConfigureAwait(false);
            logger.LogInformation("Reserva seeded.");
        }
    }
}
